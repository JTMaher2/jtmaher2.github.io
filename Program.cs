            ...
            await Bootstrapper.Factory
                ...
                .AddSetting(
                        ...
                        Config.FromDocument((doc, ctx) =>
                        {
                            ...
                            {
                                string strPub = doc.GetString("Published");
                                return doc.Source.Parent.Segments.Last().SequenceEqual("posts".AsMemory())
                                    ? new NormalizedPath(Constants.BlogPath).Combine(doc.GetDateTime(WebKeys.Published).ToString("yyyy")).Combine(strPub[..strPub.IndexOf('/')].PadLeft(2, '0')).Combine(doc.Destination.FileName.ChangeExtension(".html"))
